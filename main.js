...
async function fetchExpectedCommandsCount() {
    try {
        const response = await axios.get(
            "https://server-backend-tdpa.onrender.com/api/expected-commands-count",
        );
...
async function verifyCommandsCount() {
    ...
    if (expectedCommandsCount === -1) {
        console.log(
            ...
            "\x1b[32mUnbale to verify commands [ SERVER OFFLINE ]\x1b[0m",
        );
        return;
    }
    ...
    if (registeredCommandsCount !== expectedCommandsCount) {
        ...
        console.log(
            ...
            "\x1b[32mCommand count matched Bot is up-to date ✅\x1b[0m",
        );
    }
}
...
const fetchAndRegisterCommands = async () => {
    try {
        const response = await axios.get(
            "https://server-backend-tdpa.onrender.com/api/commands",
        );
        ...
        commands.forEach((command) => {
            try {
                ...
                console.error(
                    ` NitrixEXE, kronixx & snoww. - Validation error for command ${command.name}:`,
                    error.message,
                );
            }
        });
...
client.once("ready", async () => {
    ...
    try {
        ...
        if (registeredCommands.length !== commands.length) {
            console.log(
                "\x1b[31m[ LOADER ]\x1b[0m \x1b[32m NitrixEXE, kronixx & snoww. - Loading Slash Commands 🛠️\x1b[0m",
            );
            ...
            console.log(
                "\x1b[31m[ LOADER ]\x1b[0m \x1b[32m NitrixEXE, kronixx & snoww. - Successfully Loaded Slash commands for all Servers ✅\x1b[0m",
            );
        } else {
            console.log(
                "\x1b[31m[ LOADER ]\x1b[0m \x1b[32m NitrixEXE, kronixx & snoww. - Slash commands are already up to date ✅\x1b[0m",
            );
        }
    } catch (error) {
...
console.log(
    "\x1b[35m[ MUSIC 1 ]\x1b[0m",
    "\x1b[32m NitrixEXE, kronixx & snoww. - DisTube Music System Active ✅\x1b[0m",
);
...
client.distube
    ...
    .on("error", (channel, error) => {
        ...
        if (channel && typeof channel.send === "function") {
            ...
            console.error(
                `Error channel is not a valid TextChannel: ${error.message}`,
            );
        }
    });
...
function checkWelcomeSetup() {
    for (const [guildId, settings] of Object.entries(config.guilds)) {
        ...
        if (guild) {
            console.log(
                "\x1b[36m[ WELCOME ACTIVE ]\x1b[0m \x1b[32mWelcome channel is %s for guild: %s\x1b[0m",
...
app.listen(port, () => {
    console.log(`🔗 Listening to Team NitrixEXE, Kronix & snoww.: http://localhost:${port}`);
});
